...
## Main Function
NgramModel <- function (inpTxt,minN, maxN, freq_cutoff = 1, rank_cutoff =3, delimiters=NULL){
...
## Step 1 : Tokenize the text provided using RWeka Tokenizer

#  create tokenizer function
  
   ## enhance later to use delimiter ---> ctl <- RWeka::Weka_control(min = minN, max = maxN, delimiters = delimiters)
    ctl <- RWeka::Weka_control(min = minN, max = maxN)
    tok <- function(x) RWeka::NGramTokenizer (x, ctl)

#  split into ngrams
    ngrams <- data.table (phrase = unlist (lapply (inpTxt, tok)))
